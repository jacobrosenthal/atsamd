...
//!
//! Upload:
//! * `cargo +nightly hf2 --release --example magic_wand_rtic --features=usb,tf`
//!
...
//! infer using test data by using one of tf_slope or tf_ring features
//!
//! Record training Data by using the tf_train feature
//! * `cargo +nightly hf2 --release --example magic_wand_rtic --features=usb,tf_train`
//!
//! Note accelerometer seems to get stuck uploading sometimes. If the green led
//! doesnt arrive, or doesn't change, remove all power and plug back in.
//!
//! Note leds timing gets wonky sometimes and give odd color, ignore that.
...
const APP: () = {
    ...
    fn main(c: main::Context) -> ! {
        ...
        let model = include_bytes!("../assets/models/magic_wand_updated.tflite");
...
// hardfault is called by panic, so this avoids our led from being lit by panic handler...
#[inline(never)]
